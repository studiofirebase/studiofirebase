...
import { ArrowLeft, Facebook, Instagram, Twitter, Save, Eye, EyeOff, Loader2 } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter } from '@/components/ui/card';
import { Label } from '@/components/ui/label';
import { Input } from '@/components/ui/input';
import { useToast } from '@/hooks/use-toast';
import { updateEnvFile } from '@/ai/flows/admin-config-flow';
...
export default function IntegrationsPage() {
    ...
    const handleSave = async (integrationId: string) => {
        const integration = integrationsConfig.find(int => int.id === integrationId);
        ...
        })).filter(u => u.value); // Only update fields that have a value

        if (updates.length === 0) {
            toast({
                variant: "destructive",
                title: "Nenhuma Credencial",
                description: `Por favor, insira pelo menos uma credencial para salvar.`,
            });
        ...
        try {
            ...
            if (result.success) {
                toast({
                    title: `Integração ${integration.name} Salva!`,
                    description: "Suas credenciais foram salvas. Recarregue a aplicação para que as mudanças tenham efeito.",
                });
            } else {
                throw new Error(result.error || "Falha ao salvar as credenciais.");
            }
        } catch (error: any) {
            toast({
                variant: "destructive",
                title: `Erro ao Salvar ${integration.name}`,
      ...
      <div className="space-y-8">
        {integrationsConfig.map((integration) => (
            <Card key={integration.id}>
                ...
                <CardFooter>
                    <Button onClick={() => handleSave(integration.id)} disabled={isLoading[integration.id]}>
